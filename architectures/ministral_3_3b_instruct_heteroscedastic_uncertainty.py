"""
Mistral3 Vision-Language Model with Heteroscedastic Uncertainty
Standalone implementation - no dependencies on other architecture files.
...
class PixtralPatchEmbedding(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        patches = self.proj(pixel_values)
...
# ============================================================================
# Heteroscedastic Output Class
# ============================================================================

@dataclass
class HeteroscedasticCausalLMOutput(CausalLMOutputWithPast):
    """
    Output class for heteroscedastic models that output both logits and log_variance.
    ...
    Args:
        ...
        log_variance: Learned log variance s = log(σ²) (batch, seq_len, 1)
...
# ============================================================================
# Heteroscedastic Vision-Language Model
# ============================================================================

class Mistral3HeteroscedasticForConditionalGeneration(PreTrainedModel, GenerationMixin):
    """
    Mistral3 Vision-Language Model with Heteroscedastic Uncertainty.
    
    Outputs both logits and learned log_variance (s = log(σ²)).
    The log_variance can be used with Monte Carlo sampling for uncertainty-aware loss.
    
    σ = exp(log_variance / 2)  ->  used in: x̂ = f + σ * ε
    """
    ...
    def __init__(self, config: Mistral3Config):
        ...
        # NEW: Heteroscedastic log_variance head
        # Outputs scalar log(σ²) per token position
        # Initialized to small values (near 0) so initial σ ≈ 1
        ...
        # Initialize log_variance_head to output near-zero values
        # This gives initial σ = exp(0/2) = 1
    ...
    def forward(
        ...
        """
        Forward pass that returns both logits and log_variance.
        
        Returns:
            HeteroscedasticCausalLMOutput with:
                - logits: (batch, seq_len, vocab_size)
                - log_variance: (batch, seq_len, 1) - learned log(σ²)
        """
        return_dict = return_dict if return_dict is not None else True
        ...
        # NEW: Heteroscedastic log_variance output
        # s = log(σ²), so σ = exp(s/2)
        log_variance = self.log_variance_head(hidden_states)  # (batch, seq_len, 1)

        # Loss is NOT computed here - external loss handler will use both logits and log_variance
        loss = None
...
Mistral3HeteroscedasticConfig = Mistral3Config
